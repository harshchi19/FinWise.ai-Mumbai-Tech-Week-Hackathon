...
import { FINANCIAL_ROUTE } from "@/constants/utils";
...
const FinancialPathFlow = () => {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const response = await fetch(FINANCIAL_ROUTE, config);
      if (!response.ok) {
  ...
  const tabs = [
    {
      ...
      suggestedAssets: [
        ...
        "Fixed Deposits",
        "Dividend-Paying primary-Chip Stocks",
        "Money Market Funds",
      ],
          ...
          <div className="flex flex-wrap gap-2">
            {sampleInputs.map((sample, index) => (
              <button
                ...
                className="px-3 py-1.5 text-sm bg-white rounded-lg border border-gray-200 hover:border-primary-400 hover:bg-primary-50 transition-all duration-200 flex items-center group"
              >
        ...
        <div className="p-6 space-y-6">
          <div className="relative">
            <textarea
              ...
              className="w-full min-h-[120px] p-5 text-gray-800 placeholder-gray-400 bg-gray-50 border border-gray-200 rounded-xl resize-none focus:ring-2 focus:ring-primary-500 focus:border-transparent transition-all duration-300"
              style={{ height: "auto" }}
            />
          </div>
